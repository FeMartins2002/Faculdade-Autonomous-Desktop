...
public abstract class BaseClient {
    ...
    protected <T> T put(String url, Object body, TypeReference<T> type) {
        HttpRequest request = HttpRequest.newBuilder()
                .uri(URI.create(url))
                .header("Content-Type", "application/json")
    ...
    protected <T> T delete(String url, Object dto, TypeReference<T> type) {
        HttpRequest request = HttpRequest.newBuilder()
                .uri(URI.create(url))
                .header("Content-Type", "application/json")
                .method("DELETE", HttpRequest.BodyPublishers.ofString(toJson(dto)))
                .build();

        return send(request, type);
    }

    protected <T> T patch(String url, Object body, TypeReference<T> type) {
        HttpRequest request = HttpRequest.newBuilder()
                .uri(URI.create(url))
                .header("Content-Type", "application/json")
