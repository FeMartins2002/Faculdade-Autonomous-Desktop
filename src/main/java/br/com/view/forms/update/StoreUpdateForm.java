...
public class StoreUpdateForm extends JFrame implements ActionListener {
    ...
    public StoreUpdateForm(StoreController controller, String[] row, JButton refresh) {
    ...
    public void actionPerformed(ActionEvent click) {
        if (click.getSource() == submit) {
            ...
            try {
                if (controller.updateStore(updateStorDTO())) {
                    clickRefresh();
                    successMessage();
                    dispose();
                }
    ...
    private UpdateStoreDTO updateStorDTO() {
        return new UpdateStoreDTO(null, Long.parseLong(row[0]), txtName.getText(), txtPhone.getText(), txtAddress.getText());
    }
