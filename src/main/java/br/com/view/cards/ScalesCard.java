...
import br.com.view.forms.create.ScaleForm;
import br.com.view.forms.update.ScaleUpdateForm;
...
public class ScalesCard extends JPanel implements ActionListener {
    ...
    public void actionPerformed(ActionEvent click) {
        ...
        if (click.getSource() == editButton) {
            if (validateSelectedRow()) {
                new ScaleUpdateForm(scaleController, getSelectedRow(), refreshButton);
            }
        }

        if (click.getSource() == completedButton) {
            handleCompleteScale();
        }

        if (click.getSource() == cancelledButton) {
            handleCancelScale();
        }
    ...
    private void handleCompleteScale() {
        if (!validateSelectedRow()) return;

        int confirm = JOptionPane.showConfirmDialog(
                this,
                "Deseja realmente finalizar esta escala?",
        ...
        scaleController.confirmedScale(id);

        refreshButton.doClick();
        JOptionPane.showMessageDialog(this, "Escala finalizada com sucesso!");
    }

    private void handleCancelScale() {
        if (!validateSelectedRow()) return;

        String motivo = JOptionPane.showInputDialog(this, "Informe o motivo do cancelamento:", "Cancelar escala", JOptionPane.QUESTION_MESSAGE);

        if (motivo == null || motivo.trim().isEmpty()) {
            JOptionPane.showMessageDialog(this, "Cancelamento abortado. Motivo obrigatório.", "Atenção", JOptionPane.WARNING_MESSAGE);
            return;
        }

        String id = table.getValueAt(table.getSelectedRow(), 0).toString();
        scaleController.canceledScale(id, motivo);

        refreshButton.doClick();
        JOptionPane.showMessageDialog(this, "Escala cancelada com sucesso!");
    }
    ...
    private String[] getSelectedRow() {
        ...
        row[1] = table.getValueAt(table.getSelectedRow(), 2).toString();
        row[2] = table.getValueAt(table.getSelectedRow(), 3).toString();
        row[3] = table.getValueAt(table.getSelectedRow(), 5).toString();
        row[4] = table.getValueAt(table.getSelectedRow(), 6).toString();
        return row;
    }
